...
# Improved extract_command function to filter and prioritize valid commands
def extract_command(ai_response):
    ...
    for match in command_match:
        # Extract command and remove unwanted text
        command = match[0] or match[1] or match[2]
        if command:
            command = command.strip()
            # Avoid invalid or unclear commands
            if not any(keyword in command.lower() for keyword in ["to", "or", "press", "replace"]):
                valid_commands.append(command)

    # Extra: Try parsing single-line commands without code blocks
...
# Run valid Linux commands safely
...
# Detect if user wants to clear the screen
...
def ai_shell():
    ...
    while True:
        ...
        # Detect if user wants to run a command
        if should_auto_execute(user_input) or any(keyword in user_input for keyword in ["list", "create", "delete", "show", "move", "copy", "install", "update", "remove", "search", "clear", "ip", "hostname", "username", "whoami", "usage", "cpu", "memory", "disk"]):
                ...
                if suggested_commands:
                    ...
                    for suggested_command in suggested_commands:
                        ...
                        if result:
                            ...
                            print("⚠️ Command execution failed. Trying next command...")
                else:
