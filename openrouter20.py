...
def extract_command(ai_response):
    ...
    for match in command_match:
        ...
        if command:
            ...
            if not any(keyword in command.lower() for keyword in ["to", "or", "press", "replace", "alternatively"]):
...
def ai_shell():
    ...
    while True:
        ...
        if should_auto_execute(user_input) or any(keyword in user_input for keyword in ["list", "create", "delete", "show", "move", "copy", "install", "update", "remove", "search", "clear", "ip", "hostname", "username", "whoami", "cpu", "disk", "usage", "status"]):
            ...
            if ai_response:
                ...
                if suggested_commands:
                    for suggested_command in suggested_commands:
                        ...
                        # Automatically execute the command without asking for confirmation
