...
def extract_command(ai_response):
    # Regex to capture only valid Linux shell commands
    command_match = re.findall(r"(?<=```bash\n)(.*?)(?=\n```)|(?<=```)(.*?)(?=\n```)|(?:^\S.*$)", ai_response, re.MULTILINE | re.DOTALL)

    # Return the first valid command
    for match in command_match:
        command = match[0] or match[1]
        if command and not command.lower().startswith("to") and not command.lower().startswith("or"):
