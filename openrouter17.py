...
def extract_command(ai_response):
    # Regex to capture shell commands in code blocks or plain text
...
def ai_shell():
    ...
    while True:
        ...
        # Check if the user wants to run a command explicitly or implicit intent detected
        if should_auto_execute(user_input) or any(keyword in user_input.lower() for keyword in ["list", "create", "delete", "show", "move", "copy", "install", "update", "remove", "search", "clear", "ip", "hostname", "username", "whoami"]):
            ...
            if ai_response:
                ...
                # If only one valid command, run it automatically
                if len(suggested_commands) == 1:
                    ...
                    if "Error executing command" in result:
                        print(f"⚠️ Command Error Detected! Asking AI for suggestions...")
                        fix_response = query_ai(f"Fix this error: {result}", "Suggest a solution for this error")
                        print(f"💡 AI Suggestion:\n{fix_response}")
                    else:
                        print(f"🔹 Command Output:\n{result}")

                # If multiple suggestions, ask which one to execute
                elif len(suggested_commands) > 1:
                    print("🔹 Multiple command suggestions detected. Which one should I run?")
                    for i, cmd in enumerate(suggested_commands, start=1):
                        print(f"{i}. {cmd}")
                    choice = input("\nSelect a command to run (1, 2, 3... or 'n' to cancel): ").strip()
                    if choice.isdigit() and 1 <= int(choice) <= len(suggested_commands):
                        selected_command = suggested_commands[int(choice) - 1]
                        print(f"\n🔹 Running: {selected_command}")
                        result = run_command(selected_command)
                        print(f"🔹 Command Output:\n{result}")
                    else:
                        print("❌ Command not executed. Try again.")
                else:
