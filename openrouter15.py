...
def extract_command(ai_response):
    ...
    # Fallback: Look for first valid command if no code block
    lines = ai_response.splitlines()
    for line in lines:
        if line.strip() and not line.startswith("#"):
            # Filter invalid responses like "Supplementary Content"
...
# ✅ FIXED: Function to check if the user wants to clear the screen
...
def ai_shell():
    ...
    while True:
        ...
        if should_auto_execute(user_input) or any(keyword in user_input for keyword in ["list", "create", "delete", "show", "move", "copy", "install", "update", "remove", "search", "clear", "ip", "hostname", "username", "whoami"]):
